...
def generate_all_results():
    ...
    for a_int in range(256):
        for b_int in range(256):
            ...
            final_Sum, fixed_value = final_stage4_calculate_output(S1, stage_3_Sum_HA_1, stage_3_Sum_HA_2, stage_3_Sum_FA_1, stage_3_Sum_FA_2, stage_3_Sum_FA_3, stage_3_Sum_FA_4, stage_3_Sum_FA_5, stage_3_Sum_FA_6, stage_3_Sum_FA_7, stage_3_Cout_FA_7)
            Exact = calculate_exact_product(a, b)
            Error = final_Sum - Exact
            NMSE = Error**2 / 2**32
            Mean_Error = Error / 2**16
            Error_Distance = abs(final_Sum - Exact)
            NMED = Error_Distance / (256*256) / (255*255)
            if Exact != 0:
                Relative_Error_Distance = Error_Distance / Exact
                Mean_Error_Distance = Relative_Error_Distance / (2**16 - 511) # 511种结果为 0
                results.append([a, b, final_Sum, Exact, Error_Distance, Relative_Error_Distance, Mean_Error_Distance, Mean_Error, NMSE, NMED])
            else:
                results.append([a, b, final_Sum, Exact, Error_Distance, None, None, Mean_Error, NMSE, NMED])
    return results, fixed_value

# # 将结果输出到 Excel
# def export_to_excel(results, fixed_value, filename=f'./数据/results_Tr_{fixed_value}.xlsx'):
#     df = pd.DataFrame(results, columns=['a', 'b', 'Final_Sum', 'Exact', 'Error_Distance', 'Relative_Error_Distance', 'Mean_Error_Distance', 'Mean_Error', 'NMSE', 'NMED'])
#     df.to_excel(filename, index=False)



# 将结果输出到 Excel
def export_to_excel(results, fixed_value, filename=None):
    if filename is None:
        filename = f'./数据/results_Tr_{fixed_value}.xlsx'
    df = pd.DataFrame(results, columns=['a', 'b', 'Final_Sum', 'Exact', 'Error_Distance', 'Relative_Error_Distance', 'Mean_Error_Distance', 'Mean_Error', 'NMSE', 'NMED'])
    df.to_excel(filename, index=False)


# 示例使用
results, fixed_value = generate_all_results()
export_to_excel(results, fixed_value)
