...
clang++ -std=c++20 \
    ...
    image_processing_kodak.cpp \
    $VERILATOR_OBJ_DIR/VSabetzadeh__ALL.cpp \
    $VERILATED_CPP \
    $VERILATED_THREADS_CPP \
    $VERILATED_TIMING_CPP \
    -o image_processing_kodak \
    $OPENCV_LIBS \
    -lpthread

# 检查编译是否成功
if [ $? -eq 0 ]; then
    echo "Compilation successful. Executable 'image_processing_kodak' created."
else
