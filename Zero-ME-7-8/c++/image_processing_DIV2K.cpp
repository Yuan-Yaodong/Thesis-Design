...
// 全局变量
VZero_ME_7_8* g_top = nullptr;

// 懒加载初始化函数
void init_approx_mult() {
    if (g_top == nullptr) {
        g_top = new VZero_ME_7_8;
    }
}
...
std::map<cv::Size, std::vector<cv::Mat>, SizeComparator> read_div2k_dataset(const std::string& directory, std::vector<std::string>& image_paths) {
    ...
    // auto perms = fs::status(directory).permissions();
    // if ((perms & fs::perms::owner_read) != fs::perms::none) {
    //     std::cout << "目录具有读取权限" << std::endl;
    // } else {
    //     std::cerr << "错误：目录没有读取权限 " << directory << std::endl;
    //     return {};
    // }
...
int main() {
    ...
    // std::cout << "DIV2K 目录路径: " << fs::absolute(div2k_directory) << std::endl;
